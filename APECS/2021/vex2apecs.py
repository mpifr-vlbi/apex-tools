...
def obs_writeScans(fd,scans,sources):
	...
	for ii in isorted:
		...
		if Tprev_end==None:
			...
			Tprev_end = T_scan - datetime.timedelta(seconds=(Lslew+Ltsys+Lrefscan+4*Lcmdmargin))
		...
		T = Tprev_end + datetime.timedelta(seconds=10) # move to source right after previous VLBI scan instead!
		...
		#Tprev_end = T
		Tprev_end = T_scan_end  # nominal end of VLBI scan, sans calibratio

...
/* Storage for ID of thread to receive data broadcasts from downconverter */

    pthread_t apexVLBIDownconverterDataThread;


    quittime = 0;
    if (argc) pname = argv[0];

    sprintf(deviceName,"IFV1");
	...
	cout << "--------------------------------------------------------------\n"; 
	cout << "syntax: apexVLBIDownconverterControl -help --help -port xxxx\n";
	cout << "--------------------------------------------------------------\n";
        cout << "-port xxxxxxx ACS Component UDP port number\n";
        cout << "-nameIFV1 xxxxxxx ACS Component UDP device name\n";
	cout << "-portIFV1 xxxxxxx VLBI downconverter UDP port \n";
	cout << "-hostIFV1 xxxxxxx VLBI downconverter ip-no or hostname  \n";
	...
	my_udp_port=atol(argv[i+1]);
      ...
      if (!strcmp(argv[i],"-nameIFV1") && i < argc-1){
         sprintf(deviceName,"%s",argv[i+1]);
      }


      if (!strcmp(argv[i],"-portIFV1") && i < argc-1){ 
	udpPortIFV1=atoi(argv[i+1]);
      }
      if (!strcmp(argv[i],"-hostIFV1") && i < argc-1){
	sprintf(udpHostIFV1,"%s\n",argv[i+1]);
      }
    }

/* Create thread to receive data broadcasts from downconverter */

    pthread_create(&apexVLBIDownconverterDataThread,NULL,VLBIDownconverterCommsThread,NULL);

...
#
# Modified version of
# http://sourceforge.net/p/pyserial/code/HEAD/tree/trunk/pyserial/serial/tools/miniterm.py#l304
# with modifications marked 'JanW'.
#
# Slightly adapted from https://bitbucket.org/jwagner313/fila10gtools
# with a somewhat longer wait time between commands during the automatic
# command file "upload" to the FILA10G, needed for timesync with GPS module.
#

import sys, os, serial, threading, datetime, time
try:
    from serial.tools.list_ports import comports
except ImportError:
    comports = None

# For CTRL+<x> codes see e.g. http://nemesis.lonestar.org/reference/telecom/codes/ascii.html
EXITCHARCTER = serial.to_bytes([0x1d])   # GS/CTRL+]
MENUCHARACTER = serial.to_bytes([0x14])  # Menu: CTRL+T
SYNCCHARACTER = serial.to_bytes([0x13])  # JanW: FILA10G time sync via CTRL+S

DEFAULT_PORT = 'COM1'
DEFAULT_BAUDRATE = 19200	    # JanW: FILA10G serial is 19200,8,n,1 
DEFAULT_RTS = None              # JanW: FILA10G is not using RTS/CTS,DTR,..
DEFAULT_DTR = None
DEFAULT_FLOWCTRL_RTSCTS = False # JanW: FILA10G uses no special flow control
DEFAULT_FLOWCTRL_XONXOFF = False
DEFAULT_FILA10G_SYNC_TIMEDELTA = 0  # JanW: FILA10G sync routine will add timedelta in seconds to computer UTC time
...
def get_help_text():
    # JanW: renamed to FILA10G and added u/upload and s/synchronize commands
    return """
--- FILA10G Serial Console Tool
--- Based on the pySerial (%(version)s) - miniterm - help
---
--- %(exit)-8s Exit program
--- %(menu)-8s Menu escape key, followed by:
--- Menu keys:
---    %(info)-7s Show serial port info
---    %(upload)-7s Upload file (prompt will be shown)
---    %(sync)-7s Synchronize FILA10G to computer time
...
""" % {
    ...
    'sync': key_description(SYNCCHARACTER),
}
...
class Miniterm(object):
    ...
    def __init__(self, port, baudrate, parity, rtscts, xonxoff, echo=False, convert_outgoing=CONVERT_CRLF, repr_mode=0, initial_commands_file=None):
        try:
        ...
        self.command_file = initial_commands_file
    ...
    def writer(self):
        ...
        locally.
        """
        self.serial.write(self.newline)  # JanW: newline to get FILA10G console string

        menu_active = False
        try:
            while self.alive:

                # JanW: get initial commands from an external command file
                if self.command_file:
                    self.serial.write(self.newline)
                    self.fileupload(self.command_file, skipcomments=True, sleeptime=1.5, progressindchar='')
                ...
                if menu_active:
                    if c == MENUCHARACTER or c == EXITCHARCTER: # Menu character again/exit char -> send itself
                        ...
                        if self.echo:
                            sys.stdout.write(c)
                    elif c == '\x15' or c in 'uU':          # CTRL+U -> upload file,  u/U -> upload file
                        sys.stderr.write('\n--- File to upload: ')
                        sys.stderr.flush()
                        console.cleanup()
                        filename = sys.stdin.readline().rstrip('\r\n')
                        # self.fileupload(filename)         # JanW: commented out 
                        self.command_file = filename        # and changed file upload to use FILA10G style upload
                        console.setup()
                    ...
                    elif c == SYNCCHARACTER:                # CTRL+S -> (JanW) time sync command for FILA10G
                        self.writer_timesync_fila10g()
                        ...
                        self.dump_port_settings()
                    # JanW: removed parity selectors S (set), M (mark) and relaced with S for FILA10G sync
                    ...
                    #    self.dump_port_settings()
                    elif c in 'sS':                         # S -> time sync FILA10G
                        self.writer_timesync_fila10g()
    ...
    # JanW: FILA10G code
    # Commands:
    #  'reset' which will first clear all FIFOs  
    #  'timesync <yyyy>-<mm>-<dd>T<hh>:<mm>:<ss>[+/-ZZ:zz]' where ZZ is time zone
    def writer_timesync_fila10g(self):
        """\
        Perform one-way time synchronization of the FILA10G by 
        sending it the current UTC time over the serial port.
        Wait until just after an integer second before sending the
        time sync, using the timestamp of the next second.
        Avoid one-minute boundaries.
        """
        print 'Time sync FILA10G...'
        #self.serial.write('reset')
        self.serial.write(self.newline)
        time.sleep(0.5)
        while True:
            dt = datetime.datetime.utcnow()
            secdecim = dt.microsecond*1e-6
            if (secdecim < 0.1):
               break
        dt = dt + datetime.timedelta(seconds=DEFAULT_FILA10G_SYNC_TIMEDELTA)
        cmd = 'timesync %04d-%02d-%02dT%02d:%02d:%02d' % (dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second)
        self.serial.write(cmd)
        self.serial.write(self.newline) 
        time.sleep(1.5)
        print "Done ('%s' sent)." % (cmd)

    """Send a file"""
    def fileupload(self,filename,skipcomments=False,sleeptime=0,progressindchar='.'):
        # JanW: this code was originally in writer(), moved it here into a new function
        if filename:
            try:
                ...
                while True:
                    ...
                    if not line:
                        continue
                    if 'auto_timesync_fila10g' in line:
                        self.writer_timesync_fila10g()
                        continue
                    if (skipcomments and line.strip().startswith('#')):
                        continue
                    self.serial.write(line)
                    self.serial.write(self.newline) # JanW: changed '\r\n' to self.newline
                    # Wait for output buffer to drain.
                    self.serial.flush()
                    if sleeptime > 0:
                        # print line
                        time.sleep(sleeptime)
                    sys.stderr.write(progressindchar)   # Progress indicator.
                sys.stderr.write('\n--- File %s sent ---\n' % filename)
            except IOError, e:
...
def main():
    ...
    group.add_option("--rtscts",
        ...
        default = DEFAULT_FLOWCTRL_RTSCTS
    )
    ...
    group.add_option("--xonxoff",
        ...
        default = DEFAULT_FLOWCTRL_XONXOFF
    )
    ...
    group.add_option("--cmd",
        dest = "initial_commands_file",
        action = "store",
        help = "optional file with FILA10G setup commands, sent before giving user console access",
        default = None
    )
    ...
    group.add_option("--lf",
        ...
        default = True # JanW: FILA10G uses just \n
    )
...
3: hex dump everything""",
    ...
    try:
        miniterm = Miniterm(
            ...
            initial_commands_file=options.initial_commands_file,
        )
    except serial.SerialException, e:
    ...
    if not options.quiet:
        sys.stderr.write('--- Console for FILA10G based on pySerial example program miniterm\n')
        ...
        sys.stderr.write("--- Help: %s followed by %s | Set FILA10G time: %s followed by %s or 's' | Quit: %s or %s and 'q' ---\n" % (
            key_description(MENUCHARACTER),
            key_description('\x08'),
            key_description(MENUCHARACTER),
            key_description(SYNCCHARACTER),

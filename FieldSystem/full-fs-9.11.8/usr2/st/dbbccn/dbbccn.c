...
#define CONTROL_FILE "/usr2/control/dbbcad.ctl"
...
int main(int argc, char * argv[])
{
  ...
  while (TRUE)    {
...
#ifdef DEBUG
    fprintf(stderr,"entering dbbccn ip[0]=%d ip[1]=%d ip[2]=%d\n",
	    ip[0],ip[1],ip[2]);
#endif
    switch (ip[0]) {
    case 0:
    ...
    memcpy(ip+3,"db",2);
#ifdef DEBUG
    fprintf(stderr,"leaving dbbccn ip[0]=%d ip[1]=%d ip[2]=%d\n",
	    ip[0],ip[1],ip[2]);
#endif
  }
}
...
#ifdef DEBUG
      printf ("Cannot open dbbc host %s port %d error %d\n",host,port,error);
#endif
      fail=FALSE;
      return error;
    }
...
{   
  ...
  /* * Get service number for socket dbbc * */ 
  socaddin.sin_port = htons(port); /* Port dbdrive's number */
#ifdef DEBUG
  (void) fprintf(stderr, "%s DEBUG:  dbbc port is %d \n", 
...
#ifdef DEBUG
      printf ("dbbccn failed to get a request buffer\n");
#endif
      ip[2] = -101;
      goto error;
    }

...
**********************************************************
DBBC_cal_ch reports the entire system total power
and gain settings in converted channels.
**********************************************************
*/
...
/*****************************/
void dbbc_cal_ch(command,itask,ip)    
/****************************/
...
long ip[5];                           /* ipc parameters */
{
      ...
      if (command->argv[0] == NULL || command->equal != '=' )
            /* command is only 'dbbc_cal_ch' without arguments */   
         {
           if (DEBUG) printf("dbbc_cal_ch.c: outbuf %s \n",outbuf);
           goto dbbc_cn;
         } 
...
/*parse:*/
	...
	ptr=arg_next(command,&ilast);
      ...
      if (out_recs >=1) {
         logitf("dbbc_cal_ch: no arguments allowed,only dbbc status request");
         goto error;   
      }  
 /* end of parsing */
...
dbbc_cn:
      *(inbuf)='\0';
      ...
      if (DEBUG)
       printf("dbbc_cal_ch:Respons from DBBC= %s\n", inbuf);
      if (ierr != 0)
          goto error; 
   exit:

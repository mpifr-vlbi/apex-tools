...
long ip[5];                           /* ipc parameters */
{
      ...
      if (command->argv[0] == NULL || command->equal != '=' )
       /*  command is  'resetall' or resetall= */   
      {
            ilen=strlen(outbuf);
            ...
            if (ierr == 0)
              ...
              {
              logitf("resetall Client communication ERROR");            
              goto error;
              }
...
parse:
	...
	strcat(outbuf,",");
      ...
      if (out_recs >= 1) {
         logitf("resetall only,  allows no arguments");
      }          

...
/* Handle each mode in a separate section */
  switch (imode) {
    ...
    case 0:             /* initialize */
      ...
      logit(buf,0,NULL);
      fs->ionsor = 0;
// TODO: why Dave added this? it never gets sent. should it be sent? probably not?
      // sprintf(buf3,"frontends(\'het230\')");  // dont send? overrides setup_shfi()?
      sprintf(buf3,"  ");
      break;
    ...
    case 1:             /* source= command */
      ...
      #if 0 //(!USE_APECS_TRACK)
         // on() itself does not appear to go on source!? -- gets cancelled, Felipe thinks because
         // of 28800 seconds too long and APECS smart to recognize below horizizon in future of on(),
         // unlike Dirk's recommendation

         // so we still send the above track() command first!
         // record Total Power to MBFITS for near infinte time -- must be cancel()'ed by station.prc postob
      n = apecs_send(afd, "on(drift='no',time=1800)\n", apecs_resp, ANTCN_STR_LEN-1);
      if (n>0) {
         logit(apecs_resp, 0, NULL);
      }
      #endif

/*
 * $Id: per_file.c 1749 2014-01-09 22:54:28Z gbc $
 *
 ...
 */
static void check_packets(Random_Picker *rp)
{
    int nl, tt, ii, fails;
    uint32_t *pkt, *end;
...
int m6sc_set_chk_opt(const char *arg)
{
    ...
    if (!strncmp(arg, "help", 4)) return(chk_help());
...
int m6sc_per_file(const char *file, int files, int verbose)
{
    ...
    check_packets(picker);
    sg_close(&work.sgi.smi);

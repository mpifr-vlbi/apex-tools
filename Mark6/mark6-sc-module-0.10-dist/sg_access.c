/*
 * $Id: sg_access.c 2217 2014-06-20 19:45:21Z gbc $
 *
...
uint64_t sg_get_vsig(uint32_t *vhp, void *orig, int verb)
{
    ...
    vdif_signature.bits.stationID    = vh->w4.stationID;
 ...
 */
static int mm_init(const char *file, SGMMInfo *smi)
{
    struct stat mm_stb;
    smi->mmfd = -1;
    smi->start = (void*)0;
 ...
 */
static void mm_term(SGMMInfo *smi)
{
    if (!smi) return;
    ...
    uint32_t lower, uint32_t upper, int offset)
{
    ...
    while (upper > lower) {
        xamen = (upper + lower) / 2;
        if (sgi->verbose>1) fprintf(stdout,
            "sgb: lower %u xamen %u upper %u\n", lower, upper, xamen);
...
{
    if (mm_init(file, &(sgi->smi))) return((SGMMInfo *)0);
...
SGMMInfo *sg_reopen(SGInfo *sgi)
{
    ...
    if (mm_init(sgi->name, &(sgi->smi))) return((SGMMInfo *)0);
    /* sg_info would be redundant */
    return(&(sgi->smi));
}

void sg_close(SGMMInfo *smi)
{
    mm_term(smi);
}
...
void sg_report(SGInfo *sgi, char *label)
{
    ...
    if (rep) fputs(rep, stdout);
    else fprintf(stdout, "%s: no report available\n", label);
}
...
{
    void *pktp, *endp;
    off_t nmpkts[4], ptrdel[4], pktcnt[4], bb, rr;
    int ii, nlft;
...
{
    void *pktp, *endp;
    off_t nmblks[4], ptrdel[4], pktcnt[4];
    int ii, nlft;
...
{
    void *pktp, *endp;
    off_t ptrdel[4], pktcnt[4], bb, rr, pp;
    int ii, nlft;

...
int main (int argc,
    ...
    if (argc < 3 || argc > MAXFILES + 2)
        {
        printf ("Usage:\n");
        printf ("gather <ifile1> ... <ifilen> <ofile>\n");
        exit (0);
        }

    nfiles = argc - 2;              // number of input files [1..MAXFILES]
    ...
    fdo = open (argv[nfiles+1], O_WRONLY | O_CREAT /* | O_DIRECT */, 
    ...
    while (ostate != CLOSED)
        ...
        if (ostate == IN_PROGRESS)
            ...
            if (rc < 0 && rc != EINPROGRESS)
                ...
                rptr[oq_ptr[oblnum].bufnum] 
                             = ++rptr[oq_ptr[oblnum].bufnum] % RB_CELLS;
                oblnum++;
                ostate = IDLING;
                did_nothing = FALSE;
                }
        ...
        if (ostate == IDLING)       // if currently idle check to see if
            ...
            if (oblnum <= min_oq_max || !inputs_open && oblnum <= max_oq_max)
                ...
                aiocbo.aio_nbytes 
                    = ibuf_len[oq_ptr[oblnum].bufnum][oq_ptr[oblnum].bufidx]-lbhead;
                aio_write (&aiocbo);
